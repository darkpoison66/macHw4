    ...
    public static void main(String[] args) {
        try{
            ...
            try {
                ...
                System.out.println("[NOTE] Adding new element, iterator should"
                + " be able to iterate one more forward.");
                System.out.println("\tThis is due to the fact that Set only "
                + "adds at the end. Thus add() does not critically alter the "
                + "structure in such a way as to interfere with iteration.");
                set.add(new Square("a1"));
                try {
                    if (!iterator.hasNext()) {
                        System.out.println("Iterator says it doesn't have next"
                        + " when it does.");
                        passedIteratorTests = false;
                    }
                    if (!iterator.next().equals(new Square("a1"))) {
                        System.out.println("Iterator next did not return the "
                        + "expected a1.");
                        passedIteratorTests = false;
                    }
                } catch (Exception e) {
                    System.out.println("Caught exception attempting to iterate"
                    + " when next element exists.");
                        passedIteratorTests = false;
                }
                System.out.println("[NOTE] Adding new element at end of "
                + "for-each.");
                c = 0;
                for (Square s : set) {
                    if (c == 3 && !s.equals(new Square("a1"))) {
                        System.out.println(
                        "Iterator didn't return the expected fourth element, "
                        + "a1.");
                        passedIteratorTests = false;
                    } else if (c == 3) {
                ...
                if (set.size() != 5 || c != 5) {
                    System.out.println("Element did not add during iteration."
                    );
                    passedIteratorTests = false;
                }
            } catch(NullPointerException n) {
